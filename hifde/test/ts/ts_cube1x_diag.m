...
for nn = 1:length(vd)
  ...
  [F,tt] = hifde3x(A,x,occ,rank_or_tol,opts);
  w = whos('F');
  fprintf([repmat('-',1,80) '\n'])
  fprintf('mem: %6.2f (MB)\n', w.bytes/1e6)
  ...
  NN(nn,1) = log2(N);
    ...
    legend('N log(N)','N','N^2','diag','diag inv','HIFx','Location','NorthWest');
    if spdiag
       title('Extract diagx via sparse apply/solves, time');
    else
       title('Extract diagx via matrix unfolding, time');
    end
    axis tight;
    ...
    if spdiag
       saveas(pic,['ts_cube1x_spdiag_time.eps'],'epsc');
    else
       saveas(pic,['ts_cube1x_diag_time.eps'],'epsc');
    end
    hold off;
    pic = figure;
    hold on;
    ...
    if spdiag
       title('Extract diagx via sparse apply/solves, accuracy');
    else
       title('Extract diagx via matrix unfolding, accuracy');
    end
    axis tight;
    ...
    set(b.XLabel, 'FontSize', 20);set(b.YLabel, 'FontSize', 20);set(b.ZLabel, 'FontSize', 20);set(b.Title, 'FontSize', 20);
    if spdiag
       saveas(pic,['ts_cube1x_spdiag_accu.eps'],'epsc');
    else
       saveas(pic,['ts_cube1x_diag_accu.eps'],'epsc');
    end
    hold off;
